...
-- Props to Lance Good for this code (most of it at least): [https://github.com/GamesLigaTv]
...
AddEventHandler('chatMessage', function(source, name, msg)
    ...
    if (name ~= realName) then 
        ...
        DropPlayer(id, "[FastCity-Anticheat]: " .. Config.Messages.ChatMessageTriggered)
    end
end)
  ...
  RegisterServerEvent(BlacklistedEvents[i])
    AddEventHandler(BlacklistedEvents[i], function()
        ...
        if Config.BanComponents.AntiBlacklistedEvent then 
            BanPlayer(id, "[FastCity-Anticheat]: " .. reason);
      ...
      DropPlayer(id, "[FastCity-Anticheat]: " .. reason)
    end)
end
...
AddEventHandler("entityCreating",  function(entity)
    ...
    if (model) then 
        if (owner ~= nil and owner > 0) then
            ...
            DropPlayer(owner, "[FastCity-Anticheat]: " .. reason);
        end
        CancelEvent()
        cancelled = true
    end
        ...
        for i = 1, GetNumResources() do
            ...
            if resource_name ~= GetCurrentResourceName() then
                for k, v in pairs({'fxmanifest.lua', '__resource.lua'}) do
                    ...
                    if data and type(data) == 'string' and string.find(data, 'acloader.lua') ~= nil then
                       -- data = data:lower()
                        local removed = string.gsub(data, 'client_script "%@FastCity%-anticheat%-master%/acloader.lua"', "")
    ...
    AddEventHandler("ANTICHEAT:CHECKRESOURCES", function(givenList)
        ...
        for _, resource in ipairs(givenList) do
            if not validResourceList[resource] then
                BanPlayer(source, "[FastCity-Anticheat]: " .. Config.Messages.UnauthorizedResources:gsub("{RESOURCE}", resource));
            end
        end
    end)
end
